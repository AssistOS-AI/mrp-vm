...
function buildRetrievalRuntime() {
  ...
  strategyRegistry.register(hdcStrategy);
  ...
  return { contextMatcher, hdcStrategy };
}

class BalancedKBPlugin {
  constructor(manifest, contextMatcher, hdcStrategy) {
    this.id = manifest.id;
    this.name = manifest.name || manifest.id;
    this.description = manifest.description || '';
    this.costClass = manifest.costClass || 'cheap';
    this.profileId = manifest.profileId || 'balanced';
    this.plannerHints = manifest.plannerHints || null;
    this.contextMatcher = contextMatcher;
    this.hdcStrategy = hdcStrategy;
    this._boundKbIndex = null;
  }
  ...
  _ensureIndexListener(kbIndex) {
    if (!this.hdcStrategy || !kbIndex || this._boundKbIndex === kbIndex) return;
    kbIndex.onChange((event, unitId) => {
      if (event === 'rebuild') this.hdcStrategy.invalidate(null);
      else if (unitId) this.hdcStrategy.invalidate(unitId);
    });
    this._boundKbIndex = kbIndex;
  }

  async retrieve(input) {
    try {
      this._ensureIndexListener(input.kbIndex);
...
export async function createPlugin() {
  ...
  const { contextMatcher, hdcStrategy } = buildRetrievalRuntime();
  return new BalancedKBPlugin(manifest, contextMatcher, hdcStrategy);
}
