  ...
  validateContextCNL(markdown) {
    ...
    for (const block of blocks) {
      ...
      if (block.fields['PhaseScopes']) {
        ...
        const rawScopes = block.fields['PhaseScopes'].value.split(',').map(scope => scope.trim()).filter(Boolean);
        if (scopes.length !== rawScopes.length) {
          errors.push({ code: 'INVALID_PHASE_SCOPE', line: block.fields['PhaseScopes'].lineNum, column: 1, field: 'PhaseScopes',
