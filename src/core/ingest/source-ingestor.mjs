...
import { MAX_NORMALIZER_INPUT_CHARS } from '../normalizer/nl-normalizer.mjs';
...
export class SourceIngestor {
  constructor(normalizer, config) {
    ...
    this.singleChunkLimit = Math.min(
      config.singleChunkLimit ?? MAX_NORMALIZER_INPUT_CHARS,
  ...
  chunk(rawText, filename) {
    if (rawText.length <= this.singleChunkLimit) {
      return [{
        chunkIndex: 0,
        text: rawText.trim(),
